...
- <mark>This has been partially updated for the Nexys Video but requires some research to make sure it's all correct and true.</mark>
- <mark>The purpose of this external clock pin is for the example of measuring the DDR3 speed</mark>
...
- Pins of  Xilinx Artix-7 FPGAs are organized into banks. Banks are identified by numbers. In the [schematics of Nexys Video](https://digilent.com/reference/_media/reference/programmable-logic/nexys-video/nexys_video_sch.pdf) we can see that pins connected to sockets on the Nexys Video belong to banks 13, 14, 15, and 35. All pins in a given bank must work on the same voltage. In the case of the Nexys Video, banks 14, and 34 work on 3.3 V.
- The problem is that MIG expects sys_clk_i to be 2.5 V, but the external 100 MHz oscillator is connected to the Nexys Video pin named R4, which is in bank 34 and thus operates on 3.3 V (and the oscillator used in the circuit is actually a 3.3 V oscillator). Therefore Vitis raises the following error when I used a pin ck_a0 (also belonging to bank 35) in the design:
...
Download [Nexys-Video-Master.xdc](https://github.com/Digilent/digilent-xdc/blob/master/Nexys-Video-Master.xdc) from the [Digilent GitHub](https://github.com/Digilent).  

Add Nexys-Video-Master.xdc as the constraints file to Vivado (window Sources, "+" button). Do not forget to check "Copy constraints file into project". We want to have a copy of the file in the project because we are going to edit it.

Open the .xdc file in the editor. Uncomment the two lines for port clk and the lines for ports ja_1 and cpu_resetn.  
